...
**Phase 7 extract (2026-09-14):** Religion / ethnicity / COB choropleths join ITL1 GSS and LAD21 codes used by the official polygons (East Midlands `E12000004` / `TLF`, Halton `E06000006`, and the rest of the published E&W LA extract). The Area Table at a geography is the painted set. Map frame is UK+Ireland; map panel is compact. UKDS UV bulk and Scotland/NI static sex × age × birthplace tables were re-probed and remain blocked / not published — Area Overviews and persons cob×age stay. Story pack is narrative only. See `data/SOURCES.md` and `docs/refresh.md`.
